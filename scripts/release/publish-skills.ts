#!/usr/bin/env bun
/**
 * Publish canonical agent-net-cli skills into khoralabs/skills.
 *
 * Clones the skills repo with SKILLS_REPO_TOKEN, replaces `skills/agent-net-cli/` wholesale,
...
export const SKILL_DEST_NAME = "skills/agent-net-cli";
export const SKILL_SOURCE_REL = "packages/cli/assets/skills/agent-net-cli";
export const PACKAGE_NAME = "@khoralabs/agent-net-cli";
export const SOURCE_REPO = "khoralabs/agent-net";
...
export type PublishSkillsResult =
  | { status: "skipped"; reason: "token_unset" }
  | { status: "unchanged" }
...
export async function publishSkills(opts: PublishSkillsOptions): Promise<PublishSkillsResult> {
  ...
  const tmp = mkdtempSync(path.join(tmpdir(), "agent-net-skills-"));
  try {
    const header = gitHttpsAuthHeader(token);
