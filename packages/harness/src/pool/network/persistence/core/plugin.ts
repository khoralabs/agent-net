...
import type { ListNetworkEventsOptions, NetworkEventsPlugin } from "../../events-plugin.ts";
...
/**
 * Network-events plugin: EventStore + per-session JSONL append.
 * Backend is injected via {@link CreateNetworkEventPersistencePluginOptions.store}.
 */
export function createNetworkEventPersistencePlugin(
  ...
  function appendJsonlLine(sessionId: string, line: string): void {
    const target = networkEventSessionJsonlPath(dataDir, sessionId);
  ...
  return {
    ...
    sessionJsonlPath(sessionId: string): string {
      return networkEventSessionJsonlPath(dataDir, sessionId);
    },
