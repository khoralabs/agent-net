 ...
 * neighbor search and namespace selection. Durable workflow steps stay in the host;
 * this module owns the pure rules.
 */

/**
 * Where the workflow may write relative to the event `namespace`.
 * - `exact` (default): write into `namespace` as given.
 * - `under`: may write into `namespace` or a child path under it.
 * - `cross`: may write into any existing namespace in the database (seed stays put).
 */
